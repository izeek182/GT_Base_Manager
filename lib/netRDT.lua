if (RDT == nil) then
    ...
    local function registerAckPacket(ackNum, port)
        ...
        _LogUtil.trace(logID, "Recived packet with ack:", ackNum, " Last PacketNum:", _NetRDT.Sockets[port].pktNum,
        ...
        if(ackDist == 0) then
            ...
            if(ackDist < _NetDefs.rdtConst.maxWindowSize and _NetRDT.Sockets[port].winOffset > (0.8*_NetDefs.congestionControl.maxPkt)) then
                ...
                _LogUtil.trace(logID,"packet Acked, with many packets in flight expanding maxPkts in flight, now:".._NetDefs.congestionControl.maxPkt)
            end
            _LogUtil.trace(logID,"packet:"..ackNum.." Acked. dequeuing")
    ...
    local function ackPacket(port)
        ...
        _LogUtil.info(logID,"nextIndex:"..nextData.." ack packet, data Received but not used"..serial.serialize(_NetRDT.Sockets[port].pktRxBuf))
    ...
    local function handleTimouts()
        for localPort, socket in pairs(_NetRDT.Sockets) do
            ...
            if (socket.winOffset > 0 and timeSince(LastTx) > _NetDefs.timeOut.resend) then
                _NetDefs.queTimeout()
                _LogUtil.trace(logID, "timeout on que resending, redusing max pktsInflight to:".._NetDefs.congestionControl.maxPkt)
                resendQue(localPort)
        ...
        for localPort, socket in pairs(_NetRDT.Sockets) do
            ...
            if(timeSince(LastTx) > _NetDefs.timeOut.genSend) then 
                ...
                if (socket.winOffset < queLen) and (socket.winOffset < _NetDefs.congestionControl.maxPkt) then
                    _LogUtil.trace(logID, "sending data from que, len:", queLen, " winOffset:", socket.winOffset,
